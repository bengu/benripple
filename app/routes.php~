...
->select(DB::raw('*, users.id as uid, COALESCE(dl.balance,0) + creditlines.trust - creditlines.balance as damoney'))
->get();

$creditlines2 = DB::table('creditlines')
->join('users as users2', 'creditlines.to', '=', 'users2.id')
...
->select(DB::raw('*, LEAST( COALESCE(dl.balance,0) + creditlines.trust - creditlines.balance, COALESCE(dl2.balance,0) + cl2.trust - cl2.balance) as myleast, users2.name as vianame, COALESCE(dl.balance,0) + creditlines.trust - creditlines.balance as viaamount'))
->get();
...
return Redirect::to('user/id/' . $userdata['from']);
}));
...
Route::post('user', function()
{
	...
	if ($user){
		...
		return Redirect::to('/user/email/' . $user['email']);
	}
	else
	return "hittade ej";
});
...
Route::post('promise', array('before' => 'auth' ,function()
{

$promise = array(
	'promise'	=> Input::get('promise'),
	'unit' 		=> Input::get('unit'),
	'id' 		=> Input::get('id')
);
$good = Good::where('id', '=', $promise['unit'])->get()->first();
$user = User::where('id', '=', $promise['id'])->get()->first();
$promise['name'] = $user['name'];
$promise['unitname'] = $good['description'];

	return View::make('promise')->with('promise', $promise);
}));


Route::get('promise/{promise?}/unit/{unit?}/user/{id?}', array('before' => 'auth' ,function($promise = null, $unit = null,$id = null)
{
...
Route::post('vpromise', array('before' => 'auth' ,function(){
     $userdata = array(
'promise' => Input::get('promise'),
'unit' => Input::get('unit'),
'id' => Input::get('id')
...
return "det kommer snart kunna modifiera i databasen";
}));
